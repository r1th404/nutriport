...
/* ─── CHATBOT WIDGET (pojok kanan bawah) ─────────────────────────────────── */
.chat-widget-panel {
    background: #FFFFFF !important;
    border: 0;
    border-radius: 16px;
...
div[data-testid="column"]:has(div.st-key-nutriport_chat_toggle),
div[data-testid="column"]:has(.chat-widget-anchor),
div[data-testid="column"]:has(.chat-widget-panel) {
    ...
    flex: 0 0 min(460px, calc(100vw - 32px)) !important;
    display: flex !important;
    flex-direction: column-reverse !important;
    align-items: flex-end !important;
    gap: 0 !important;
}
div[data-testid="column"]:has(.chat-widget-anchor) > div,
div[data-testid="column"]:has(.chat-widget-panel) > div {
    width: 100% !important;
}
div[data-testid="column"]:has(.chat-widget-panel) {
    background: #FFFFFF !important;
...
div.st-key-nutriport_chat_toggle {
    width: auto !important;
    margin-left: auto !important;
}
div.st-key-nutriport_chat_toggle button {
    ...
    color: #2563EB !important;
    border: 1px solid #BFDBFE !important;
    box-shadow: 0 12px 30px rgba(30,58,95,0.2) !important;
    font-size: 1rem !important;
    font-weight: 800 !important;
}
div.st-key-nutriport_chat_toggle button:hover {
    box-shadow: 0 14px 34px rgba(30,58,95,0.28) !important;
    transform: translateY(-1px);
}
div.st-key-nutriport_chat_widget {
    display: none;
}
div.st-key-nutriport_chat_widget [data-testid="stPopover"] > div:first-child {
    display: none;
}
div.st-key-nutriport_chat_widget button {
    display: none !important;
}
div.st-key-nutriport_chat_widget button:hover {
    box-shadow: 0 12px 28px rgba(37,99,235,0.48) !important;
    transform: translateY(-1px);
}
div[data-testid="stPopoverBody"] {
    ...
    border: 1px solid #DBEAFE !important;
    box-shadow: 0 18px 44px rgba(30,58,95,0.18) !important;
...
NUTRIPORT_GUIDE = """
Nutriport adalah aplikasi monitoring kualitas menu Program Makan Bergizi Gratis (MBG) untuk sekolah.

Alur penggunaan aplikasi:
1. Login — Operator memasukkan kode sekolah pada halaman login, lalu klik "Masuk ke Dashboard" untuk membuka dashboard sekolah tersebut.
2. Tab "Laporan" di dashboard:
   - Upload foto menu makan hari ini (disarankan dari sudut atas/top-down agar semua komponen terlihat jelas).
   - Klik tombol "Analisis Kelengkapan Menu" untuk menjalankan deteksi otomatis berbasis model YOLO.
   - Sistem mendeteksi 5 komponen menu: Makanan Pokok, Lauk/Protein, Sayur, Buah, dan Susu, lalu menghitung estimasi porsi visual berdasarkan rasio luas bounding box komponen terhadap luas tray/nampan.
   - Sistem menghasilkan Skor Kelengkapan (0-100) dan status menu (Layak/Cukup/Kurang/Tidak Layak) berdasarkan komponen yang terdeteksi dan estimasi porsinya.
   - Klik "Buat Rekomendasi AI" untuk mendapatkan catatan evaluasi otomatis dari AI (memakai Groq LLM, dengan fallback rule-based jika API tidak tersedia).
   - Setelah hasil deteksi dan rekomendasi AI tersedia, klik "Kirim Laporan Hari Ini" untuk mengirim laporan ke dashboard monitoring.
3. Tab "Histori" di dashboard:
   - Menampilkan kartu metrik: persentase kepatuhan laporan hari kerja, jumlah hari belum lapor, rata-rata skor kelengkapan, dan jumlah hari dengan menu lengkap 100%.
   - Menampilkan grafik tren kepatuhan laporan per hari dan grafik ketersediaan tiap komponen menu.
   - Menampilkan tabel riwayat laporan harian (14 hari terakhir).

Catatan penting yang harus selalu disampaikan jika relevan:
- Estimasi porsi bersifat VISUAL/heuristic berbasis luas area pada foto, bukan pengukuran berat, volume, atau kandungan gizi yang akurat.
- Hasil deteksi sangat dipengaruhi kualitas foto; sebaiknya foto diambil dari atas, pencahayaan cukup, dan semua komponen menu terlihat dalam satu frame.
- Jika ada komponen yang sebenarnya ada tetapi tidak terdeteksi, operator disarankan mengunggah ulang foto dengan posisi komponen yang lebih jelas, lalu menganalisis ulang.
""".strip()

CHAT_SYSTEM_PROMPT = f"""
Kamu adalah "Asisten Nutriport", chatbot bantuan/guideline untuk aplikasi web Nutriport (monitoring menu Program Makan Bergizi Gratis/MBG).
Tugasmu menjawab pertanyaan operator sekolah/SPPG seputar CARA MENGGUNAKAN fitur-fitur di aplikasi ini saja.

Berikut panduan lengkap fitur aplikasi yang harus kamu jadikan acuan utama:
---
{NUTRIPORT_GUIDE}
---

Aturan jawaban:
- Jawab dalam Bahasa Indonesia, singkat, ramah, dan langsung ke poin (maks 5-6 kalimat atau beberapa poin singkat).
- Jika pertanyaan di luar topik aplikasi Nutriport (misalnya pertanyaan umum, resep, atau topik tidak terkait), jawab sopan bahwa kamu hanya bisa membantu seputar penggunaan aplikasi Nutriport.
- Jangan mengklaim data gizi/kalori akurat; tegaskan bahwa estimasi porsi bersifat visual jika relevan.
- Jangan mengarang fitur yang tidak ada di panduan di atas.
""".strip()


def rule_based_chat_fallback(user_message):
    """Fallback FAQ rule-based jika GROQ_API_KEY tidak tersedia atau panggilan API gagal."""
    text = (user_message or "").lower()

    def has(*keywords):
        return any(k in text for k in keywords)

    if has("login", "masuk", "kode sekolah"):
        return ("Untuk masuk, masukkan kode sekolah Anda di halaman login lalu klik "
                "**\"Masuk ke Dashboard\"**. Setelah berhasil, Anda akan diarahkan ke dashboard sekolah tersebut.")
    if has("upload", "unggah", "foto"):
        return ("Foto menu diunggah di tab **Laporan**. Gunakan foto dari sudut atas (top-down) dengan "
                "pencahayaan cukup agar semua komponen menu (makanan pokok, lauk, sayur, buah, susu) terlihat jelas, "
                "lalu klik **\"Analisis Kelengkapan Menu\"**.")
    if has("deteksi", "yolo", "analisis"):
        return ("Klik **\"Analisis Kelengkapan Menu\"** setelah upload foto. Sistem akan mendeteksi 5 komponen menu "
                "secara otomatis (model YOLO) dan menghitung estimasi porsi visual berdasarkan luas komponen pada foto.")
    if has("skor", "porsi", "kelengkapan"):
        return ("Skor Kelengkapan (0-100) dihitung dari komponen menu yang terdeteksi dan estimasi porsi visualnya. "
                "Perlu diingat, ini estimasi visual dari foto, bukan pengukuran gizi/berat yang akurat.")
    if has("rekomendasi", "ai", "llm", "groq"):
        return ("Setelah hasil deteksi muncul, klik **\"Buat Rekomendasi AI\"** untuk mendapatkan catatan evaluasi "
                "otomatis (ringkasan, hal yang perlu diperhatikan, dan rekomendasi untuk SPPG/sekolah).")
    if has("kirim", "laporan"):
        return ("Setelah ada hasil deteksi dan rekomendasi AI, klik **\"Kirim Laporan Hari Ini\"** di tab Laporan "
                "untuk mengirim laporan ke dashboard monitoring.")
    if has("histori", "dashboard", "riwayat", "grafik", "tren"):
        return ("Tab **Histori** menampilkan kepatuhan laporan, rata-rata skor kelengkapan, grafik tren laporan, "
                "grafik ketersediaan komponen menu, dan tabel riwayat laporan 14 hari terakhir.")
    return ("Saya bisa membantu seputar penggunaan Nutriport, misalnya: cara login, cara upload & analisis foto menu, "
            "arti skor kelengkapan, cara membuat rekomendasi AI, cara kirim laporan, atau cara membaca tab Histori. "
            "Silakan tanyakan salah satu topik tersebut ya 🙂")


def generate_chatbot_reply(user_message, history):
    """Hasilkan balasan chatbot guideline dengan Groq. Fallback ke rule-based jika API tidak tersedia/gagal."""
    api_key = get_secret_or_env("GROQ_API_KEY")
    model_name = get_secret_or_env("GROQ_MODEL", "llama-3.3-70b-versatile")

    if not api_key:
        return rule_based_chat_fallback(user_message)
...
def render_chatbot_widget():
    ...
    with st.container(key="nutriport_chat_fixed"):
        ...
        st.markdown('<div class="chat-widget-panel">', unsafe_allow_html=True)
        if True:
            st.markdown('<div class="chat-widget-header">Asisten Panduan Nutriport</div>', unsafe_allow_html=True)
            st.markdown(
                '<div class="chat-widget-sub">Tanyakan cara pakai fitur di web ini — login, upload foto, '
                'analisis menu, rekomendasi AI, sampai kirim laporan.</div>',
                unsafe_allow_html=True,
            )

            if not st.session_state.chat_messages:
                st.markdown(
                    '<span class="chat-widget-suggest">Cara upload foto menu?</span>'
                    '<span class="chat-widget-suggest">Apa arti skor kelengkapan?</span>'
                    '<span class="chat-widget-suggest">Cara kirim laporan?</span>',
                    unsafe_allow_html=True,
                )

            history_html = '<div class="chat-widget-history">'
            for msg in st.session_state.chat_messages:
                bubble_class = "chat-bubble-user" if msg["role"] == "user" else "chat-bubble-bot"
                safe_content = html.escape(msg["content"]).replace("\n", "<br>")
                history_html += f'<div class="chat-bubble {bubble_class}">{safe_content}</div>'
            history_html += "</div>"
            st.markdown(history_html, unsafe_allow_html=True)

            with st.form("nutriport_chat_form", clear_on_submit=True):
                user_input = st.text_input(
                    "Tulis pertanyaan...",
                    key="nutriport_chat_input",
                    label_visibility="collapsed",
            ...
            if submitted and user_input.strip():
                ...
                with st.spinner("Asisten sedang menjawab..."):
                    reply = generate_chatbot_reply(user_input, st.session_state.chat_messages[:-1])
